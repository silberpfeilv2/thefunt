...
module.exports.baslat = async (client, message, args) => {
    ...
    client.veritabanı.ayarla(`${message.guild.id}.ayarlar.prefix`, args.prefix)

    if (args.prefix === 'varsayılan' || args.prefix === 'default') {
        if (client.veritabanı.varMı(`${message.guild.id}.ayarlar.prefix`) === false) return message.send(new Discord.RichEmbed()
            .setColor("#FF0000")
            .setDescription('Sunucunuzda prefix varsayılan olarak ayarlı. Sıfırlayabileceğim bir şey yok.'));

        client.veritabanı.sil(`${message.guild.id}.ayarlar.prefix`)
