    ...
    module.exports = {
        ...
        args: [
         {
					anahtar: 'member',
					soru: 'Kimin sunucuda konuşma engelini kaldırmak istersin?',
					tip: 'kullanici'
				},
				{
					anahtar: 'sebep',
					soru: 'Neden bu kişiyi sunucuda konuşma engelini kaldırmak istiyorsun?',
					tip: 'yazi'
				}
        ]
    };
...
if (!message.guild) {
 ...
 .setFooter(client.ayarlar.footer, client.ayarlar.footerURL) 
  .addField('Hata!', '`konuştur` adlı komutu özel mesajlarda kullanamazsın.')
...
//if(message.author.highestRole.comparePositionTo(args.member.highestRole) > 0){
		...
		if (!message.guild.members.get(args.member.id).kickable) return msg.channel.send(client.ayarlar.basarisiz + ' Bu kişinin engelini kaldıramıyorum çünkü `benden daha yüksek bir role sahip` ya da `bana gerekli yetkileri vermedin`.');
    	if (args.member.id === msg.author.id) return msg.channel.send(client.ayarlar.basarisiz + ' Kendini konuşturamazsın.')
     ...
     .setFooter(client.ayarlar.footer, client.ayarlar.footerURL)
      ...
      .setTitle("Sunucuda Konuşma Engelini Kaldırma")
      .setDescription("Siz" + " ("+args.member.tag+") "+message.author+" ("+message.author.tag+") tarafından " + args.sebep + " konuşma engeliniz kaldırıldı.")
     ...
     .setFooter(client.ayarlar.footer, client.ayarlar.footerURL)
      .setImage(args.member.avatarURL)
      .setTitle("Sunucuda Konuşma Engelini Kaldırma")
      .addField("Engeli Kaldırılan:", args.member + " ("+args.member.tag+")", true)
      .addField("Kaldıran:", msg.author + " ("+msg.author.tag+")", true)
